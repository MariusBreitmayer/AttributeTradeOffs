...
Have you ever been at the gas station and wondered how much fuel your car would need if all the innovation achieved over the last 25 years have improved fuel economy instead of engine power characteristics, weight or other characteristics?
...
Using data from 1980 to 2006, we will reproduce the estimates for technological trade-offs that manufacturers and consumers face when choosing between fuel economy, weight and engine power characteristics. We will also examine how the relationship between these different factors have changed over that time.
...
Here we will take a look at how policy can incentivise car manufacturers to produce cars with a better fuel economy. This will help us to estimate if the actions had a positive effect on fuel economy. 
...
Here we will take a closer look at the technological progress. The goal is to understand how technological progress is mesured and how fuel economy could look like in 2006 if we held other characteristics constant at their level of 1980.
...
To select cars only, and get a first impression on the variable `mpg` (which will later be also refered as fuel economy), click *check*.
...
What this means is, that maybe there has been an incentive early in the sample to incease fuel economy over other characteristics such as horsepower.
...
After getting an idea how the manufacturers average non saled weighted fleet looked like over the years, we might now be interested in how CAFE Standards have changed compared to the average fuel economy.
...
Our correlation coefficient is `0.8058689` and eventhough our sample is quiet small, this suggests a strong positive linear relationship. 
...
After getting a first idea of our data and how policy might create incentive for manufacturers to increase their fuel economy in exercises one and two, we will get get a deeper understanding how different car attributes influence fuel economy in this exercise. 
...
First of, since we still want to know a little more about our data, we will create some density plots.
...
We can see that the fuel economy density in 1980 is slimmer than in 2006. This means that in 1980 the fuel economy was more similar between cars than in 2006. Another point to mention is, that the peek has shifted from around 18 miles per gallon in 1980, to roughly 26 miles per gallon in 2006. Because the density is wider in 2006, we can assume that customers have more options to give their preferences to fuel economy when buying a car. Another thing to notice is, that the density for 23 miles per gallon is roughly the same in 1980 and 2006. This means, that there are almost the same amount of cars that are capable of going 23 miles per gallon in 1980 and 2006. The main difference though is, that in 1980 23 miles per gallon has been on the top half of fuel economy, while in 2006 23 miles per gallon is bottom half of fuel economy. 
...
It is very interesting that in 1980 a car with more than 200 horsepowers has been almost a rarity. Most of the cars had between 80 and 180 horsepowers. In 2006, 200 horsepowers can almost be considered as a standard amount of horsepower for a new car.   
...
Therefore we could assume, that there might be a realtionship between these values and fuel economy. We will take a look at this in later exercises. 
...
One of the problems is, that we can not control the size of $Z_{it}$. 
...
### c) Model 1
After getting an overview of the data in earlier exercises and getting an idea on how the models work in exercise a), we will now use the knowledge in order to impove the introducing model. 
...
As you might see, some of these effects can not be influenced by manufacturers or customers.
...
In order to only have the values we need, we wil extract them from the results. For this task the `tidy()` command from the package broom helps us to make this task more convenient.
...
Data frames are more easily precessed, reshaped or combined with with tools from other packages like for example 'dplyr', 'tidyr' or 'ggplot2'. 
...
If you want to see what is stored in `TECH_PROG_MOD1`, you have to simply uncommend the last line in this command.
...
If you would like to see how `TECH_PROG_MOD2` looks, just umcommend the last line of code.
...
In oder to plot both of the results, we will first create a data frame containing the values of `TECH_PROG_MOD1` and `TECH_PROG_MOD2`.

As in previous exercises, if you would like to see how `p12` looks, just umcommend the last line of code.
...
# This command saves the coefficients into a dataframe called datreg1
...
So the value we get from our regression is `3.106287`, which is equal to the mean values of this year. We can see that the results of our models are equal to the real value, therefore our calculation appers to be correct.  
...
We can see that the market penetration of superchargers started in 1988. Since then it has increased with a small decrease in 1996. In the years after 1996, we can see that more and more care have been equipped with a supercharger.  
...
What we can see here is, that the market penetration of new cars regarding turbochargers has increased drastically early in the sample. After a big downfall between 1989 and 1996, the amount of new cars equipped with a turbocharger inceased drastically again. 

Now, regarding both market penetrations, we can say that more and more care are equipped with a supercharger or turbocharger, especially in the later years of our sample. 
...
#< info "Translog"
The first form of a translog production may be considered the proposal made in 1967 by J. Kmenta. 
...
Since you now know again which characteristics are part of the Cobb-Douglas model, this is how the level sets of our translog model looks like:
...
Afterwards we will calculate the absloute incrase in `lmpg` for every model. 
...
* Wooldridge, Jeffrey M. (2013). Introductory Econometrics: A Modern Approach (Fifth international ed.). Australia: South-Western. ISBN:978-1-111-53104-1 pages p. 44, 92, 303 and 852
...
* Stock, J. H., Watson, M. W. (2007): Introduction to Econometrics. Second Edition, Boston: Pearson Education Inc. ISBN: 978-0-321-27887-6 pages 13, 21, 27, 68, 93, 350-351, 356
...
* Yacobucci, Bamberger (2008): Automobile and Light Truck Fuel Economy: The CAFE Standards. page CRS-3 Internet: [link](http://www.hsdl.org/?view&did=716410) 28.11.15
...
* Florin - Marius PAVELESCU (2011): Some aspects of the translog production function estimation,  [link](http://revecon.ro/articles/2011-1/2011-1-8.pdf)
...
* David Robinson (2015). broom: Convert Statistical Analysis Objects into Tidy Data Frames. R package version 0.4.0
[https://cran.r-project.org/web/packages/broom/index.html](https://cran.r-project.org/web/packages/broom/index.html)
...
* William Sribney, StataCorp (1998): Comparison of standard errors for robust, cluster, and standard estimators [link](http://www.stata.com/support/faqs/statistics/standard-errors-and-vce-cluster-option/), 20.11.15
